...
func (m *Middleware) AccessForManager(c *gin.Context) {
	...
	if !exists {
		response.NewErrorResponse(c, http.StatusUnauthorized, "roles empty", "failed to get role")
		return
	}

	if role != "manager" && role != "cco" && role != "root" {
		response.NewErrorResponse(c, http.StatusForbidden, role.(string), "access not allowed")
		logger.Error("", logger.AnyAttr("userID", userId))
		return
	}
}

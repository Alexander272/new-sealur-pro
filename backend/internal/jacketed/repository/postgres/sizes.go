...
func (r *SizeRepo) GetDn(ctx context.Context, req *models.GetDnDTO) ([]*models.Dn, error) {
	query := fmt.Sprintf(`SELECT DISTINCT(dn), dn_alt FROM %s AS s WHERE flange_id=$1 ORDER BY dn_alt`,
		SizeTable,
	)
...
func (r *SizeRepo) Get(ctx context.Context, req *models.GetSizeDTO) ([]*models.Size, error) {
	query := fmt.Sprintf(`SELECT id, dn, dn_alt, pn, pn_alt, d4, d3, d2, d1
		FROM %s WHERE flange_id=$1 AND dn_alt=$2 ORDER BY count`,
		SizeTable,
	)

	data := []*models.Size{}
	if err := r.db.SelectContext(ctx, &data, query, req.FlangeId, req.Dn); err != nil {
		return nil, fmt.Errorf("failed to execute query. error: %w", err)
	}
	return data, nil
}
...
func (r *SizeRepo) Create(ctx context.Context, dto *models.SizeDTO) error {
	query := fmt.Sprintf(`INSERT INTO %s (id, count, dn, dn_alt, pn, pn_alt, d4, d3, d2, d1, flange_id) 
		VALUES (:id, :count, :dn, :dn_alt, :pn, :pn_alt, :d4, :d3, :d2, :d1, :flange_id)`,
		SizeTable,
	)

...
func (r *FlangeTypeRepo) Get(ctx context.Context, req *models.GetFlangeTypeDTO) ([]*models.FlangeType, error) {
	query := fmt.Sprintf(`SELECT id, title, code FROM %s WHERE standard_id=$1 ORDER BY code`, SerratedFlangeTypeTable)
...
func (r *FlangeTypeRepo) Create(ctx context.Context, dto *models.FlangeTypeDTO) error {
	query := fmt.Sprintf(`INSERT INTO %s (id, title, code, standard_id) VALUES ($1, $2, $3, $4)`, SerratedFlangeTypeTable)
...
func (r *FlangeTypeRepo) Update(ctx context.Context, dto *models.FlangeTypeDTO) error {
	query := fmt.Sprintf(`UPDATE %s SET title=$1, code=$2, standard_id=$3 WHERE id=$4`, SerratedFlangeTypeTable)
...
func (r *FlangeTypeRepo) Delete(ctx context.Context, dto *models.DeleteFlangeTypeDTO) error {
	query := fmt.Sprintf(`DELETE FROM %s WHERE id=$1`, SerratedFlangeTypeTable)

...
import (
	...
	"github.com/Alexander272/new-sealur-pro/internal/wave/transport/http/wave_type/base"
...
type Handler struct {
	service services.WaveType
}

func NewHandler(service services.WaveType) *Handler {
	return &Handler{service: service}
}

func Register(api *gin.RouterGroup, services *services.Services, middleware *middleware.Middleware) {
	handler := NewHandler(services.WaveType)

	waveType := api.Group("/types")
	{
		waveType.GET("", handler.get)
		write := waveType.Group("", middleware.CheckAccess(constants.AllowAdmin))
		{
	...
	base.Register(waveType, services.WaveTypeBase, middleware)
}
...
func (h *Handler) delete(c *gin.Context) {
	...
	if err := h.service.Delete(c, &models.DeleteWaveTypeDTO{Id: id}); err != nil {

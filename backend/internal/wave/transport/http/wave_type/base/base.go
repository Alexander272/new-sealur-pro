...
type Handler struct {
	service services.WaveTypeBase
}

func NewHandler(service services.WaveTypeBase) *Handler {
	return &Handler{
		service: service,
	}
}

func Register(api *gin.RouterGroup, service services.WaveTypeBase, middleware *middleware.Middleware) {
	handler := NewHandler(service)

	waveTypeBase := api.Group("/base")
	{
		waveTypeBase.GET("", handler.get)
		write := waveTypeBase.Group("", middleware.CheckAccess(constants.AllowAdmin))
		{
...
func (h *Handler) get(c *gin.Context) {
	data, err := h.service.Get(c, &models.GetWaveTypeBaseDTO{})
	if err != nil {
...
func (h *Handler) delete(c *gin.Context) {
	...
	if err := h.service.Delete(c, &models.DeleteWaveTypeBaseDTO{Id: id}); err != nil {

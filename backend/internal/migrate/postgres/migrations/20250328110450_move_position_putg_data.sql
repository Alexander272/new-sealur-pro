...
INSERT INTO public.position_putg(
	id, position_id, putg_standard_id, flange_type_id, configuration_id, size_id, h, d4, d3, d2, d1, filler_id, 
	...
	SELECT ps.id, ps.position_id, putg_standard_id, flange_type_id, configuration_id, COALESCE(size_id, '00000000-0000-0000-0000-000000000000'),
	...
	LEFT JOIN LATERAL (SELECT id AS size_id, d4, d3, d2, d1, h
		FROM putg_size LEFT JOIN LATERAL (SELECT construction_id FROM putg_construction WHERE id=mat.construction_id) AS c ON true
		WHERE flange_type_id=m.flange_type_id AND base_construction_id=construction_id AND dn=ps.dn AND pn=ps.pn_mpa) AS s ON true;
-- +goose StatementEnd

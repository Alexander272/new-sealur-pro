...
INSERT INTO public.position_snp(
	id, position_id, snp_standard_id, snp_type_id, flange_type_id, size_id, pn_index, h_index, another, filler_id, 
	...
	COALESCE(pn_index, -1), COALESCE(h_index, 0), ps.h, filler_id, frame_id, inner_ring_id, outer_ring_id,
	...
	LEFT JOIN LATERAL (SELECT id AS size_id, array_position(pn_mpa, ps.pn_mpa) AS pn_index, array_position(h, ps.h) AS h_index, 
		d4, d3, d2, d1, h FROM snp_size WHERE snp_type_id=m.snp_type_id AND dn=ps.dn AND ps.pn_mpa=ANY(pn_mpa)) AS s ON true;
-- +goose StatementEnd

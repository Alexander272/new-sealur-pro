...
CREATE TABLE IF NOT EXISTS public.serrated_size
(
    id uuid NOT NULL,
    ...
    CONSTRAINT serrated_size_pkey PRIMARY KEY (id),
    CONSTRAINT serrated_size_type_id_fkey FOREIGN KEY (type_id)
        REFERENCES public.serrated_type (id) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE CASCADE
        NOT VALID
)
...
ALTER TABLE IF EXISTS public.serrated_size
    OWNER to postgres;
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
DROP TABLE IF EXISTS public.serrated_size;
-- +goose StatementEnd

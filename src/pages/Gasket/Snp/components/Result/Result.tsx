...
export const Result: FC<Props> = () => {
	...
	//TODO можно попробовать сделать шаблон по типу "Прокладка СНП-{type}-{filler}-{d2}-{pn}-{thickness}" только проблема с необязательными полями (надо как-то понимать что необязательное и куда его вставлять) можно заключать их в квадратные скобки (только по условиям надо подумать как сделать) + смотри заметку в телефоне

	//TODO а что мне мешает создать таблицу для шаблона. и там описать порядок формирования шаблона, а так же указать обязательное поле из нет и условия если они нужны
